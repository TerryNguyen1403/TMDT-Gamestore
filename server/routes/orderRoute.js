...
router.post("/create_payment_url", async function (req, res, next) {
  ...
  const newOrder = {
    ...
    status: 0, // Chưa thanh toán, mới chỉ lưu order tạm
    totalAmount: totalAmount,
  };

  const result = await Order.create(newOrder);

  // Cập nhật lại orderId
  if (result) {
...
router.get("/vnpay_return", async function (req, res, next) {
  ...
  if (secureHash === signed) {
    ...
    const orderId = vnp_Params["vnp_TxnRef"]; // set ở bước tạo đơn hàng
    const rspCode = vnp_Params["vnp_ResponseCode"]; // mã phản hồi

    if (orderId && rspCode === "00") {
      ...
      if (updated) {
        ...
        await Cart.deleteOne({ userId: updated.userId });
      }
    }
  ...
  res.redirect(
    `${clientUrl}/vnpay_return?` + qs.stringify(vnp_Params, { encode: false })
  );
});
